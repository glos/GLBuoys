...
function initialize(jsonObj) {
    ...
    //Create legend everytime map section is toggled open. Div is removed in loadbuoyinfo if div exist when section is toggled. 
...
function loadbuoyinfo(ID, jsonObj) {
                ...
                if (jsonObj[i].id == ID) {
                    ...
                    if (jsonObj[i].obsUnits) {
                        ...
                        if (!isNaN(ID.charAt(0))) {
                            $('#textBuoy').append('<p><strong><font color="red">New Number:</font></strong>&nbsp;Text ' + jsonObj[i].id + ' to <a  id="SMS" href="sms:1-734-201-0750">(734) 201-0750</a> for the latest observations.</i></p>');
                    ...
                    $('#buoyLocation h4').css("cursor", "pointer");
                    $('#buoyLocation h4').click(function () { toggleSection('googleMap'); initialize(jsonObj); if ($('#googleMapLegend')) { $('#googleMapLegend').remove() }});
                    $('#buoyLocation').append('<div id="googleMap" class="w3-hide"><style type="text/css">div#map_canvas { margin: 0 auto 0 auto; }</style><div id="map_canvas" style="width:100%;max-width:400px;height:350px;margin: 0 auto"></div></div>');
	...
	callfooterInfo(ID);
    //initialize(jsonObj);  Load map be default
}
