    ...
    //}
		...
		//	var jsonObj = JSON.parse(response);
			...
			for (i = 0; i < stations.length; i++) {
                ...
                var contentString = '<div id="googleMapcontent" style="cursor: pointer;font-family: Inconsolata,Verdana; font-size:15px; color:#333;font-weight:700" onclick="PassStation(\'' + stations[i] +'\',\'' + lats[i] +'\',\'' + lons[i] + '\');dataLayer.push({\'event\':\'glbuoysEvent\',\'glbuoysCategory\':\'map\',\'glbuoysLabel\':\''+stations[i]+'\',\'glbuoysAction\':\'click_internal_url\'});">' + stations[i] + '</div>';
                map.setCenter({
                    lat: lats[i],
                    lng: lons[i]
                })
                ...
                infowindow.open(map, marker);
					...
					google.maps.event.addListener(map, 'zoom_changed',(function (marker, i){
						...
						var infowindow2 = new google.maps.InfoWindow({
							position: new google.maps.LatLng(lats[j],lons[j]),
...
$(document).ready(function () {
    ...
    var ID;
		//if (sessionStorage.getItem("station")!= null){
			//ID = sessionStorage.getItem("station");
			////loadbuoyinfo(ID);
				...
				if (jsonObj[i].id == ID) {
					...
					if(jsonObj[i].obsUnits){
                        ...
                        if (jsonObj[i].webcamSrc.length > 0) {
                            ...
                            //$('#BuoyCamPic').append('<p>(Click image to view video clip at third party site.)<p>');
